...
**TODO на наступну сесію: протестувати на реальному файлі** — маленький тестовий набір (5-10 рядків) з навмисно різними випадками: чиста локація, множинні значення через кому, свідомо "биту" статус-строку — щоб перевірити, що `failed_import_rows` реально ловить помилки, а решта рядків імпортується коректно.
